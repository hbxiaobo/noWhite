...
export default {
  ...
  methods: {
    ...
    //  查看排行榜
    show_top_api (callback) {
      $.post('?m=apiGame&type=tops', {}, function (datas) {
        var data = datas.data
        var $html = ''
        $('.rank-ul').html('')
        $html += '<li>'
        $html += '<span class="order" style="font-size: 12px; line-height: 80px;">序号</span>'
        $html += '<span class="head-img" style="font-size: 12px; line-height: 80px;">头像</span>'
        $html += '<span class="nickname" style="font-size: 12px; line-height: 80px;">昵称</span>'
        $html += '<span class="score-bar" style="font-size: 12px; line-height: 80px;">分数</span>'
        $html += '</li>'
        for (var i = 0; i < data.length; i++) {
          $html += '<li>'
          $html += '<span class="order" style="font-size: 12px; line-height: 80px;">' + data[i].rank + '</span>'
          $html += '<span class="head-img" style="font-size: 12px; line-height: 80px;">'
          $html += '<img src="' + data[i].headimgurl + '">'
          $html += '</span>'
          $html += '<span class="nickname" style="font-size: 12px; line-height: 80px;">' + data[i].nickname + '</span>'
          $html += '<span class="score-bar" style="font-size: 12px; line-height: 80px;">' + data[i].score + '</span>'
          $html += '</li>'
        }
        $html += '<li>'
        $html += '<span class="order" style="font-size: 12px; line-height: 80px;"></span>'
        $html += '<span class="head-img" style="font-size: 12px; line-height: 80px;">'
        $html += '</span>'
        $html += '<span class="nickname" style="font-size: 12px; line-height: 80px;"></span>'
        $html += '<span class="score-bar" style="font-size: 12px; line-height: 80px;"></span>'
        $html += '</li>'
        $('.rank-ul').html($html)
